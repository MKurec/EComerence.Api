...
namespace EComerence.Infrastructure.Services
{
   public class OrderListService : IOrderListService
   {
      ...
      public async Task AddOrderAsync(Guid userId, Guid productId, short amount)
      {
         ...
         if (orderList == null)
         {
            ...
            orderList.AddProduct(product, (ushort)amount);
            await _orderListRepository.AddAsync(@orderList);
         }

         if (orderList.Orders.Keys.Any(x => x.Id == productId))
         {
            if (amount > 0)
            {
               @orderList.UpdateOrder(product, (ushort)amount);
            }
            else if (amount == -1)
            {
               var key = orderList.Orders.Where( x => x.Key.Id == productId).FirstOrDefault();
               var currentAmount = orderList.Orders[key.Key];
               orderList.UpdateOrder(product, (ushort)(currentAmount + 1));
         ...
         {
            orderList.AddProduct(product, 1);
            await _orderListRepository.SaveAsync(@orderList);
         }
      ...
      public async Task<int> GetCurrentCountItemsAsync(Guid userId)
      {
         var orderLists = await _orderListRepository.BrowseAsync(userId);
         var orderList = orderLists.FirstOrDefault(x => x.Purchased == false);
         if (orderList == null)
         {
            return 0;
         }
         var count = orderList.GetAmountOfProducts();
         return count;
      }
